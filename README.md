...
The React desktop window is built into `dist/windows/desktop/desktop.html`, which is the file declared in `manifest.json`.
...
The app starts from `windows/background/background.html`, then opens the built React desktop window declared in `manifest.json`.
...
Open `http://127.0.0.1:5173/desktop.html`. The Overwolf APIs will be reported as unavailable in browser preview mode.
...
- `dist/windows/desktop/` contains the built desktop UI generated by Vite.
